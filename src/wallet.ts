...
export function handleTransaction(
  ...
  if (wallet != null) {
    let currentNonceResult = walletInstance.try_nonce();
    if (currentNonceResult.reverted) {
      log.warning("handleTransaction:: Call reverted for function: nonce", []);
      return;
    }

    const currentNonce = currentNonceResult.value;
    let nonce = currentNonce.equals(zeroBigInt())
      ? currentNonce
      : currentNonce.minus(oneBigInt());
    let txHashResult = walletInstance.try_getTransactionHash(
      to,
      value,
      data,
      operation,
    ...
    if (txHashResult.reverted) {
      log.warning(
        "handleTransaction:: Call reverted for function : getTransactionHash",
        []
      );
      return;
    }

    let txHash = txHashResult.value;
    ...
    if (data.length < 2700) {
      ...
      log.warning(
        "wallet: {} transaction {} - cannot store transaction.data (too long), length: {}",
        [
          walletAddr.toHexString(),
          hash.toHexString(),
